...
| 日期 | 事件 |
|------|------|
| 2026-06-08 |原本第一个项目想做agent工作室是为了给claude code，codex等不同家agent建立一个互相沟通且能一起与用户聊天的渠道。希望能一边推动项目，一边学习agent互相协调。但是经过7天烧掉12亿token以后，我发现agent本身还不足以支持严格的团队协作框架，长时间不断压缩上下午会让情况更加糟糕。这可能也是为什么当前很多人在研究不是长期训练agent做的更好，而是把agent完全当工具，去研究怎么一直能回滚agent到适合当前任务的最佳状态。
因此，我决定把工作室本身的项目和训练agent作为团队协同得进行分开。项目完全应有的功能就行了。而agent协同或许可以引用最新的Hermes看看效果。|
